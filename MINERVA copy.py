...
from zoomPixmap import ZoomableGraphicsView
...
class LoadingPage(QMainWindow):
    def __init__(self):
        ...
        self.loading_label.setGeometry(0, 0, 300, 300)
...
class MainWindow(QMainWindow):
    ...
    def fusion_process(self):
        ...
        if self.MetodeVGG:
            self.vgg_gen = vggGen(self.folder_inputFusion1, self.folder_inputFusion2, self.outputNameFusion, self.extensionFileFusion, self.folder_fusionOutput_path)
            self.vgg_gen.startFusion.connect(self.update_textbox)
            self.vgg_gen.viewFusionOut.connect(self.previewFusedImg)
            self.vgg_gen.finishedFusion.connect(self.update_textbox)
            self.vgg_gen.finishedFusion.connect(self.finishFuse)
            self.vgg_gen.start()
    ...
    def update_image4(self, recMatrixFolder):
        ...
        # Membuat QLabel untuk menampilkan gambar
        self.rec1_label = ZoomableGraphicsView(pixmap3)
        self.rec1_label.setAlignment(Qt.AlignCenter)
        #self.rec1_label.setText("No Image")
        self.rec1_label.setStyleSheet("border:3px solid #E9ECEF; border-radius: 5px; font-family: Montserrat; font-weight: bold; font-size: 12pt;")
        #self.rec1_label.setScaledContents(True)
