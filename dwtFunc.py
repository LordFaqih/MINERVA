...
def fusion(img1, img2, name, extension, outPath):
    # Params
    ...
    outImage = np.round(outImage).astype(np.uint8)
    fusionPath = f"{outPath}/{name}.{extension}"
    
    cv2.imwrite(fusionPath, outImage)